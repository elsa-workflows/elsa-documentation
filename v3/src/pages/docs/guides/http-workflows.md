...
In this guide, we'll take a look at a workflow that can receive HTTP requests, make HTTP requests and write back to the HTTP response object.

---

## Handling inbound HTTP requests

When you create ASP.NET web applications, you typically create one or more API endpoints. that may receive a payload via the HTTP request body using model binding, perform some operations like data access, and finally return a response.
...
With Elsa, you now have yet another option to implement APIs by creating workflows that start with the **HTTP Endpoint** activity.  

### HTTP Endpoint activity

The HTTP endpoint activity effectively makes your workflow invokable via HTTP requests. It lets you specify a request path to handle, one or more HTTP verbs, and even lets you specify which .NET type to parse inbound JSON payloads into.
